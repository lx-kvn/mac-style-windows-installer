...
**步驟 8 驗不到這件事**（2026-09-09 發現，這裡原本寫著它可以）：那一關跑的是
`verify_release_build`，而它做的是**靜默**安裝——靜默模式的既定行為正是不顯示
EULA（`/S` 視同已經同意）。要確認那一頁真的出現，只能在客體桌面上真的把安裝檔
打開再截圖，見步驟 8c。
...
上面兩關都看不到這件事：8a 做的是靜默安裝（`/S` 視同已同意，不顯示 EULA），
8b 跑的是測試套件與打包流程，不開安裝介面。而忘了帶步驟 6 的 `--config` 時，
安裝檔會安靜地跳過那一頁、不報錯（`eula_texts` 為空時的行為就是不顯示）——
只有親眼看過才知道。

作法是在客體桌面上真的把安裝檔打開，等視窗出來再截圖：

```python
vm = vms.connect("win11", purpose="確認安裝檔會顯示授權合約頁")
with vms.preserved_tab(vm.machine.vmx):
    vms.fresh_boot(vm)
    vm.copy_in(setup_exe, r"C:\Users\Tester\Setup.exe")
    # no_wait 是必要的：安裝檔停在等人按的畫面上，等它結束會兩邊互等到逾時
    vm.run_program(r"C:\Users\Tester\Setup.exe", interactive=True,
                   no_wait=True, check=False)
    time.sleep(45)          # onefile 自我解壓 + WebView2 開窗
