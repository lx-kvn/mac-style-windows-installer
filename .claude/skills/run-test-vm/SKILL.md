...
- 消費者版（家用版／專業版）的 Windows 10 1809 安裝媒體已無官方管道可取得，
  其行為無法以此環境驗證。
- 兩台系統皆未啟用。不影響 MSIX 部署，個人化設定被鎖。**但畫面右下角
  沒有出現未啟用浮水印**（2026-09-07 實際觀察）——「設定」顯示無法連上
  網路、因而無法確認授權狀態，浮水印未出現可能與此有關，未查證。這件事
  影響的是截圖能不能直接給人看：目前可以。

## 已寫好的驗證情境

- `tools/verify_release_build.py`——發布產物的煙霧測試：把編好的
  `Setup_...exe` 送進客體，靜默安裝、確認落地內容與 PATH、實際執行裝好的
  CLI、再靜默移除並確認清乾淨。`/released` 的步驟 8 會呼叫它。
- `tools/verify_msix_1809.py`——1809 上的 MinVersion 部署與側載預設值。
- `tools/drive_installer_gui.py`——**互動畫面**：在客體桌面上以 SendInput
  移動真正的游標，把應用程式圖示拖到安裝目的地，確認那個手勢真的觸發安裝。
  判準只認檔案有沒有落地，截圖是給人看的附件。
- `tools/measure_msix_scope.py`——全機器使用者範圍的行為量測（ADR-0013）。
  兩輪之間會還原快照：佈建紀錄無法從系統介面移除。
- `tools/verify_upgrade_install.py`——**更新覆蓋安裝**：裝了舊版之後再裝
  新版，驗磁碟與登錄表上的結果（舊版檔案有沒有真的消失、有沒有留下重複的
  登錄表項目或備份資料夾、PATH 有沒有被加第二次、升級之後還移不移得掉）。
  **這一關 CI 結構上做不到**：`upgrade.py` 對「舊版項目在 HKCU 且行程已提權」
  的組合有一道安全防護會直接拒絕，而 runner 永遠是提權的。
- `tools/verify_eula_gate.py`——**授權合約頁**：那一頁只出現在互動安裝，
  靜默安裝定義上就跳過它。以「擋不擋得住」為判準，三輪（沒有合約的對照組、
  有合約直接拖、按下同意再拖）。
- `tools/verify_uninstall_drag.py`——**解除安裝的拖曳**：把應用程式圖示拖
  到垃圾桶，確認那個手勢真的會移除，而且落點是有作用的（誘餌那一輪拖到視窗
  裡的空白處，東西應該還在）。拖完之後再按一下完成畫面的「完成」，整個安裝
  目錄才會被自我刪除的背景指令清掉——那一步順便驗到了完成畫面。介面上唯一
  能移除程式的動作走的就是這條路，既有的自動化全部走 `--silent`。
- `tools/verify_msix_all_users.py`——全機器使用者範圍**實作完成後**的驗收
  （同一份 ADR）：提權的子行程真的佈建了嗎、雜湊不符擋不擋得住、靜默安裝
  未提權時降不降級、已提權時直不直接佈建。同樣每輪之間還原快照。
  **UAC 那個視窗不在它的範圍內**——安全桌面不接受合成輸入，`SendInput`
  按不到它，因此改為分別驗證它兩側的東西。

**寫這類腳本時的兩個實測心得**：`FindWindow` 以精確標題尋找中文標題的
視窗會找不到（連續 150 秒回傳 0，而同時 `EnumWindows` 列出的清單裡就有
那個標題）——改用列舉比對。另外，找不到目標時要把桌面上實際有的視窗
標題一併回報，否則分不出「還沒開」與「標題對不上」。

## 待辦事項

- 解除安裝端的拖曳（拖到垃圾桶）尚未比照辦理。

## 已完成之待辦

- 拖曳的**手感**（跟手程度、磁吸、彈回）——2026-09-09 由使用者在真實視窗上
  實際操作確認可接受。ADR-0002 記載這一項只能由人驗證，自動化涵蓋的是「那個
  手勢會不會觸發安裝」，兩者是不同的問題。
