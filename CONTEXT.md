...
- **拖曳安裝** — 把 App 圖示拖到安裝目的地圖示上放開，藉此觸發安裝。這是安裝
  流程的核心動作，也是這個專案模仿 macOS DMG 的識別所在，實作在 `ui/index.html`。

拖曳安裝這一段的組成：

- **拖曳本體** — 使用者手上那張跟著游標移動的 App 圖示（`#drag-item`）。
- **原位殘影** — 拖曳本體被拿起後，留在原本位置上的視覺替身，用來表達「這東西
  仍然屬於這裡，只是暫時被拿起來」，同時作為沒有命中時彈回去的目的地。
- **安裝目的地圖示** — 接收拖曳本體的資料夾圖示（`#drop-target`），同時也是
  更改安裝路徑的入口（點它會開啟選擇資料夾對話框）。
- **命中判定** — 放開的瞬間，判斷這次拖曳算不算「放進去了」。採用**重疊即命中**：
  拖曳本體與安裝目的地圖示的範圍有重疊就算命中，不要求游標本身壓在目的地上。
  不採用依放開速度往前推算落點的做法（那適用於可回頭的動作，安裝會實際寫入
  使用者系統，不應該因為手一抖甩過去就觸發）。

鍵盤操作走的是同一段安裝邏輯，但不播放拖曳動畫（見 `tests/test_ui_accessibility.py`）。
