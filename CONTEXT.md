...
（已定案的個別決策）；已經做完的調查與功能規劃見 `docs/investigations/`
（例如 `MSIX輸出規劃.md`），還沒拍板、仍在研究階段的則見 `docs/proposals/`
（目前是空的）。文件放置的完整慣例見 `CLAUDE.md`「文件放哪裡」一節。
...
- [安裝路徑與使用者範圍](#安裝路徑與使用者範圍)
  - [佈建與註冊（全機器範圍底下的兩個動作）](#佈建與註冊全機器範圍底下的兩個動作)
...
使用者）。`no_admin_install` 在該模式下不表達使用者範圍，也不表達安裝路徑
...
全機器範圍不是「一次裝給所有人」，而是兩個分屬不同權限、不同時機的動作。
講的時候要分開講，混用會使敘述自相矛盾：

- **佈建**（provision）——**管理員**把套件登記到這台機器上，使每一位使用者
  「有資格」取得它。做完的當下沒有任何人真的裝好。需要系統管理員權限。
- **註冊**（register）——**某一位使用者**那一端真正完成安裝，這個人才用得到。
  不能在提升權限的行程中進行（提升後的行程取不到該使用者的套件生命週期
  環境，見 [ADR-0013](docs/adr/0013-msix-all-users-scope-is-an-opt-in-field.md)
  的補記）。

兩者的關係在實測中確認：佈建完成的**當下**執行者尚未取得該應用程式，系統會
在其後自行替他補上註冊；其他使用者則於自己那一端完成註冊之後才取得。註冊
完成之前，該使用者的檔案關聯不生效。

這個分別直接決定了安裝流程的形狀：佈建交由提權的子行程執行，主行程留在未
提權狀態完成註冊。
