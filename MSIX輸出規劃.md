...
3. **WinRT 綁定套件的可行性驗證**：`winsdk`/`pywinrt` 這類套件能不能在
   ...
   路線 B 說明），不能只測「呼叫成功、裝得起來」就算過關——如果進度
   回報接不到，現有拖拽介面的進度條會被迫退化，這個結果會影響到要不要
   繼續走這條路線的判斷。
4. **MSIX 套件本身怎麼產生**：這個工具現在的輸入是「使用者選一個資料夾
   當 app 內容」，MSIX 需要一份 `AppxManifest.xml` + 用 `makeappx.exe`
   打包——這一步的產生邏輯要另外設計，不是把現有的檔案複製邏輯改個
   輸出格式就好。
5. **簽章政策要不要開人工核准**：SignPath（跟同類雲端代簽服務）把
   「正式發版簽章」設計成建議要有人工核准這道關，跟「測試簽章可以全
   自動」是分開的政策。要不要為了讓 CI 全程無人值守而放棄這道保護、
   還是接受 CI 要暫停等維護者手動核准，這個決定會影響下游專案 CI 的
   實際寫法（見上方「下游專案的 CI 建置順序」一節），必須先定案。
