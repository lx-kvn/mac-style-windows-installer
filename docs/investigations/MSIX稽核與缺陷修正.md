...
- [A1 與 A4 讓 D1 那一類缺口不會再發生](#a1-與-a4-讓-d1-那一類缺口不會再發生)
  ...
  - [驗收](#驗收-5)
- [S1 簽章憑證的密碼經由命令列參數傳遞](#s1-簽章憑證的密碼經由命令列參數傳遞)
  - [重現方式](#重現方式-4)
  - [機制](#機制-4)
  - [方案評估](#方案評估-4)
  - [一個實測推翻的預設](#一個實測推翻的預設-1)
  - [實際做了什麼](#實際做了什麼-4)
  - [驗收](#驗收-6)
- [實機驗收：S1 與 D3 各自跑過一次完整流程](#實機驗收s1-與-d3-各自跑過一次完整流程)
  - [S1：存放區模式真的簽了東西](#s1存放區模式真的簽了東西)
  - [D3：降版在真的機器上跑過](#d3降版在真的機器上跑過)
  - [一個只有實機才顯現的缺口](#一個只有實機才顯現的缺口)
  - [驗收](#驗收-7)
- [已知限制](#已知限制)
- [待辦清單](#待辦清單)
- [已完成之待辦](#已完成之待辦)
...
嚴重程度：中。暴露位置為打包機器，而打包機器正是存放簽章憑證的所在。

### 重現方式

於 `signing` 設定憑證後執行 `pack`，簽章那一刻 `signtool` 的命令列為：

```
signtool sign /f <憑證路徑> /p <密碼> /fd sha256 /tr <時間戳記> /td sha256 <目標>
```

同一台機器上的任何行程皆可讀取其他行程的命令列（WMI 的 `Win32_Process`、
Sysmon 的行程建立事件、多數端點防護產品亦會將其寫入日誌）。

### 機制

本專案在其他位置對此已有防範：密碼不寫入設定檔（[ADR-0004](../adr/0004-inline-install-password-is-gui-only.md)）、
簽章失敗的錯誤訊息不帶入密碼變數（`builder._sign_file()` 的註釋載明「避免哪天
有人手滑加進去」）。命令列這個出口的暴露面大於前兩者，而它一直存在。

`signtool` 未提供以環境變數或標準輸入傳遞 `.pfx` 密碼的選項，因此「換一種傳
密碼的方式」不成立——要讓密碼離開命令列，只能讓那一次簽章根本不需要密碼。

### 方案評估

決定經 `/grill-with-docs` 逐項確認，完整理由記於
[ADR-0014](../adr/0014-signing-certificate-is-identified-by-thumbprint-only.md)，
此處只列結論與未採用的選項：

- **採用**：新增憑證存放區模式（`signing.cert_thumbprint`），私鑰由作業系統
  保管，`signtool` 以 `/sha1` 指定，命令列上沒有密碼。檔案模式保留。
- **不採用「只以主體名稱片段指認」**（`signtool /n`）：存放區中有兩張符合時
  `signtool` 不報錯、逕自選一張。選錯的後果在 MSIX 模式下是終端使用者裝不
  起來，且錯誤訊息不指向此原因——`msix_settings` 的 `subject.mismatch` 訊息
  即為防止該情形而寫。
- **不採用「直接移除檔案模式」**：雲端代簽等憑證不在本機的情境未必能改用
  存放區模式，且移除為對外契約的破壞性變更。
- **不採用「保留檔案模式但不告知」**：S1 的核心正是「使用者不知道該出口
  存在」，保留而不告知等同僅完成一半的修正。

### 一個實測推翻的預設

`cert_store.list_signing_certificates()` 最初只依「有沒有私鑰」篩選。實測
開發機的個人存放區：**194 張憑證**，絕大多數為 Fiddler 之類的工具產生的 TLS
憑證。全部列出使用者無從找出自己那一張，而那些憑證交給 `signtool` 亦簽不了
東西。

改以加強金鑰使用方法（EKU）篩選——沒有宣告 EKU 的不受用途限制可以用，有宣告
的須含程式碼簽章（`1.3.6.1.5.5.7.3.3`）或「任何用途」（`2.5.29.37.0`）——之後
剩 8 張。

此項若未實際執行一次，單元測試不會顯現：測試斷言的是「回傳一個清單」與
「每一項都帶齊欄位」，194 張與 8 張同樣通過。

### 實際做了什麼

1. 新增 `cert_store.py`：指紋的正規化與形式檢查、兩個個人存放區的檢索、
   `signtool` 參數的推導（依實際找到的位置決定要不要帶 `/sm`）。
2. `packaging_core._validate_signing_config()` 接受 `cert_thumbprint`，兩種
   來源互斥；指紋於**清空 `dist/`、`build/` 之前**即確認存在且有私鑰
   （[ADR-0003](../adr/0003-allow-prerelease-suffix-in-version-string.md)
   決定四建立的慣例）。找到的憑證隨設定帶到簽章那一步，不重複查詢。
3. `builder._certificate_arguments()`：存放區模式印出憑證主體（指紋人不易
   辨識，ADR-0014 決定二接受該代價的條件即為這一行輸出）；檔案模式印出密碼
   會出現在命令列上的提醒。
4. `packaging_core._read_signing_cert_subject()` 於存放區模式改從找到的憑證
   讀主體，使 `msix.certificate_subject` 的自動填入在該模式下繼續成立。
5. 新增 `list-certs` 子指令與配置精靈的下拉選單。GUI 使用者因此完全不需要
   接觸那四十個字元——會用配置精靈的人通常正是不想碰命令列的那一群，只加一個
   貼指紋的欄位，等於把安全的那條路留給命令列使用者專用。
6. 三個模組共用的 Win32 憑證結構收進 `cert_subject.py` 一份。`authenticode`
   原本自行宣告了一組，再加 `cert_store` 即為三份，而欄位順序或大小其中一份
   寫錯的症狀是讀到別的欄位的位元組。

### 驗收

新增測試 51 項：`tests/test_cert_store.py` 28 項（指紋的三種形式、形式檢查、
清單的欄位與去重、EKU 篩選的六種情形、找不到與形式不合法的處置、`signtool`
參數不含密碼）；`tests/test_packaging_core.py` 11 項（兩種來源互斥、指紋
正規化、不存在與無私鑰於打包階段被擋下、憑證隨設定帶走、發行者自動填入的
兩條路）；`tests/test_builder.py` 5 項（存放區模式的參數不含 `/p` 與 `/f`、
本機電腦的憑證帶 `/sm`、兩種模式各自的建置紀錄輸出）；
`tests/test_builder_cli.py` 8 項（`list-certs` 的輸出內容與空存放區的處置）；
`tests/test_gui_signing_certificate.py` 14 項（後端只送辨識用欄位、讀不到
存放區不讓表單停住、前端的二選一與下拉選單、只送選定的來源）。

**介面實測**：配置精靈的兩種模式皆實際執行並截圖確認——存放區模式的下拉選單
確實列出本機憑證，切換至檔案模式後顯示原本的欄位，兩者的說明文字皆未截斷。

全套測試 `1638` 項通過。

## 實機驗收：S1 與 D3 各自跑過一次完整流程

2026-09-05。前面 S1 與 D3 兩節的測試全數為注入替身，兩者的完整路徑在此之前
從未真的發生過——存放區模式沒有簽過任何一個檔案，`msix_deploy.remove()` 沒有
執行過。本次稽核已兩度被「實際跑一下」推翻預設（`WTD_SAFER_FLAG`、憑證清單的
194 張），因此補上這一輪。

### S1：存放區模式真的簽了東西

四個關卡逐一以真實的存放區與真實的 `signtool` 走過：

| 關卡 | 結果 |
| --- | --- |
| `validate_and_build_pack_data()` 收下帶空格的指紋並正規化 | 通過 |
| `msix.certificate_subject` 自動填入 | 與憑證主體逐字相符 |
| 真的 `signtool` 接受 `/sha1` 並簽章 | 成功 |
| 簽出來的檔案讀得出簽章者 | 即該張憑證 |

第四關的 `WinVerifyTrust` 回報 `0x800B0109`，那是自簽憑證未匯入信任存放區的
預期結果，不是簽章沒簽上——改以「讀不讀得出簽章者」確認。

其後並以 `build_all()` 編出一顆完整的 MSIX 引擎 `Setup.exe`（23.9 MB），
`.msix` 與 `exe` 皆以存放區模式簽章。這是該模式第一次走完整條打包路徑。

### D3：降版在真的機器上跑過

於 Windows 11 25H2（26200、zh-TW）：先以未提權的行程安裝 1.1.0.0，再執行
內嵌 0.9.0.0 的 `Setup.exe`。

- **互動安裝**：降版警示如期跳出，內容含兩個版本號與「資料會被清除」那一句。
- **靜默安裝**：如 ADR-0015 決定三所訂直接執行，機器上的版本自
  `MswiAudit.D3Probe_1.1.0.0_x64__n893y0bs55h76` 變為
  `MswiAudit.D3Probe_0.9.0.0_x64__n893y0bs55h76`。`msix_deploy.remove()`
  至此第一次真的執行。

### 一個只有實機才顯現的缺口

首次靜默降版的 `/LOG=` 紀錄檔**只有一行成功訊息**，ADR-0015 決定三所要求的
「舊版連同其資料將被移除」不在其中。

成因是兩份紀錄互不相通：`msix_install` 的 `report()` 寫的是安裝檔內部的
`install_log.txt`，而 `/LOG=` 指定的那一份由 `run_silent_install()` 自行維護。
決定三要求的是後者。

單元測試涵蓋不到這一點：既有的斷言檢查的是 `log` 收到什麼，而 `log` 收到的
確實是對的——錯的是那個東西沒有到達使用者指定的檔案。

**處置**：降版的告知一併放進 `msix_install.run()` 的回傳值，走
`result["warnings"]` 這條既有的路——`run_silent_install()` 早就會把它逐條寫進
紀錄檔（F01 建立的慣例）。發現這個缺口時，那個出口就已經在那裡了。

修正後重跑同一輪實機驗證，紀錄檔中出現該段告知，安裝結果不變。

### 驗收

新增測試 2 項（降版的告知出現在回傳值裡、一般安裝不帶該告知）。
全套測試 `1640` 項通過。
...
- S1 的檔案模式行為未變，選用該模式者的密碼仍會出現在命令列上。
- 存放區模式的指紋隨憑證續約而變更，設定檔須一併更新（ADR-0014 決定二
  所接受的代價）。
- 將 `.pfx` 匯入個人存放區的動作由使用者自行完成，本工具不代勞。
  ADR-0005 決定二所承諾的**信任**存放區寫入輔助指令至今仍未實作，該項
  與本次的個人存放區是兩件不同的事。

## 待辦清單

本次稽核發現的項目皆已處理完畢，無待辦。仍然存在但屬於稽核範圍之外的
限制記於上方「已知限制」。
...
- D3 已安裝的 MSIX 套件沒有被偵測（2026-09-05，偵測、實機量測與更正說明
  文字皆已完成；降版的處置另見下方 ADR-0015 那一項）。
- S2 下載回來的載入器執行前沒有驗證（2026-09-05）。
- A1 每一個打包能力都必須被分類過（2026-09-05）。
- A4 內嵌內容與設定檔的描述由同一個值推導（2026-09-05）。
- D3 的降版處置（2026-09-05，ADR-0015）：部署前先偵測並比版本，降版經
  使用者同意後移除再裝，靜默安裝直接做，發行者不同的同名套件只告知。
- S1 簽章憑證的密碼經由命令列參數傳遞（2026-09-05，ADR-0014）。
- S1 與 D3 的實機驗收（2026-09-05）：兩者的完整路徑各自跑過一次，並修正
  一個只有實機才顯現的缺口（靜默降版的告知沒有進到 `/LOG=` 紀錄檔）。
