...
- [機制分析](#機制分析)
- [方案評估](#方案評估)
  - [一、偵測對象：工具自己的行程，或編譯安裝檔的那個直譯器](#一偵測對象工具自己的行程或編譯安裝檔的那個直譯器)
  - [二、攔截點：純函式內部，或呼叫端](#二攔截點純函式內部或呼叫端)
  - [三、hidden import 是否必要](#三hidden-import-是否必要)
  - [四、相依宣告的形式](#四相依宣告的形式)
- [實際做了什麼](#實際做了什麼)
- [驗收](#驗收)
- [驗收過程另外發現、一併處理的三項](#驗收過程另外發現一併處理的三項)
  - [一、CI 的傳統引擎測試設定自 2026-08-29 起即為無效組合](#一ci-的傳統引擎測試設定自-2026-08-29-起即為無效組合)
  - [二、build_msix() 未清理組裝目錄](#二build_msix-未清理組裝目錄)
  - [三、CLI 把訊息裡的標籤原樣印在終端機上](#三cli-把訊息裡的標籤原樣印在終端機上)
- [已知限制](#已知限制)
- [待辦](#待辦)
- [已完成之待辦](#已完成之待辦)
...
- **GitHub Actions（第二次，run 33733654851）**：下一節兩項處置完成後重跑，
  ...
  至此有兩份互相獨立的反證。
- **缺少 `winrt-*` 的環境**（2026-09-03）：以 `python -m venv --without-pip`
  建一個只裝 `pywebview`、不裝 `winrt-*` 的直譯器，把它放在 `PATH` 最前面
  （工具本身仍以原本的直譯器執行，被探測的才是那個乾淨環境），實際執行
  `builder_cli.py pack`：

  ```
  msix_backend_found = False   ready = True
  拒絕編譯：
  MSIX 引擎的安裝檔需要 `winrt-*` 綁定套件才能呼叫 Windows 的套件部署介面，⋯⋯
  exit=1
  ```

  結束碼 1，`dist/`、`.msix`、`msix_staging/` 均未產生——攔截確實發生在動手
  之前。同一環境下傳統引擎的 `missing_engine_dependencies()` 回傳 `None`，
  不受影響。這一項原列為已知限制與待辦，至此完成。
...
### 三、CLI 把訊息裡的標籤原樣印在終端機上

在缺少 `winrt-*` 的環境實跑攔截時，終端機出現的是：

```
請先執行 <code>pip install -r requirements.txt</code> 再打包
```

`builder_cli._strip_html()` 只認得 `<br>`，因為它寫成的當下訊息表裡只有
`<br>`。後來新增帶其他標籤的訊息不會有任何地方報錯——症狀只出現在使用者的
終端機上。本輪新增的 `msix.missing_dependency` 與既有的
`password.missing_dependency` 都帶 `<code>`，兩者同樣受影響。

處置：`_strip_html()` 改為去掉所有標籤並保留標籤包住的文字（那段文字通常
正是要照做的指令）。測試斷言的是「所有模組的訊息表經過該函式之後不含任何
標籤」，而不是列舉目前用到哪幾種標籤——後者在下次有人用到第三種標籤時
照樣會通過。訊息表裡沒有非標記用途的角括號，因此通用的去標籤不會誤傷。
...
1. 在未安裝 `winrt-*` 的環境實際跑一次打包，確認攔截如預期發生。已於
   2026-09-03 完成，做法與結果見「驗收」第三項。
